...
class Assignment1:
    ...
    def interpolate(self, f: callable, a: float, b: float, n: int) -> callable:
        ...
        if np.all(ys_finite):
            ...
            # Detect large dynamic range more aggressively (general-purpose)
            huge = False
            pos = ys[ys > 0]
            if pos.size >= 2:
                y_ratio = float(np.max(pos) / np.min(pos))
                log_span = float(np.log(np.max(pos)) - np.log(np.min(pos)))
                huge = (y_ratio > 1e6) or (log_span > 12.0) or (abs_max > 1e6)
            else:
                huge = (abs_max > 1e6)

            if huge and np.all(ys > 0):
                # Try log / loglog
                logy = np.log(ys)
                ...
                if np.all(logy > 0):
                    ...
                    if span_log > 0 and span_loglog < 0.8 * span_log:
                        def forward(y):
                ...
                # Mixed signs / zeros / super huge: asinh scaling
